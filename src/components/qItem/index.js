...
class QuestionsItem extends Component {
  ...
  renderViews() {
    if(this.props.item.view_count < 1000){
      return (
        <Text style={{width: '30%', textAlign: 'right', color: 'black', fontWeight: '600',  fontSize: 19, paddingTop: 5}}>{this.props.item.score}<Text style={{ fontWeight: '500',fontSize: 14, color: 'grey', marginTop:2 }}> views</Text></Text>
      )
    } else if(this.props.item.view_count <10000) {
      return(
        <Text style={{width: '30%', textAlign: 'right', color: 'black', fontWeight: '600',  fontSize: 19, paddingTop: 5}}>{this.props.item.score/1000}K<Text style={{ fontWeight: '500',fontSize: 14, color: 'grey', marginTop:2 }}> views</Text></Text>
      )
    } else {
      return(
        <Text style={{width: '30%', textAlign: 'right', color: 'black', fontWeight: '600',  fontSize: 19, paddingTop: 5}}>{this.props.item.score/1000000}M<Text style={{ fontWeight: '500',fontSize: 14, color: 'grey', marginTop:2 }}> views</Text></Text>
      )
    }
  }
         ...
         <View 
          style={{
            ...
            <View style={{flexDirection: 'row', margin: 10,}}>
              ...
              <View style={{ flexDirection: 'column' , marginLeft: 5}}>
                <View style={{ flexDirection: 'row' }}>
                  <Text style={{ fontSize: 18, color: 'black', fontWeight: 'bold', marginRight: 5}}>{this.props.item.owner.display_name}</Text>
                  <Text style={{ fontSize: 14, color: 'grey' , marginTop: 4}}>{this.props.item.owner.reputation}</Text>
                </View>
                <Text style={{ fontSize: 14, color: 'grey' }}>on {moment(new Date(this.props.item.creation_date*1000).toString()).format('MMM DD,YYYY')} at {moment(new Date(this.props.item.creation_date*1000).toString()).format('HH:mm')}</Text>
              </View>
            </View>
            ...
            <View style={{ flexDirection: 'row' ,width: '90%', marginLeft: 15, borderTopWidth: 1, borderColor: 'lightgrey', paddingTop: 20, marginBottom: 20}}>
                {this.renderVotes()}
                <View style={{width: '40%', textAlign: 'center', borderRadius: 5, backgroundColor: '#228B22',  paddingBottom: 5, paddingTop: 5}}>
                <Text style={{textAlign: 'center', color: 'white', fontSize: 18}}>{this.props.item.answer_count} answers</Text>
                </View>
                {this.renderViews()}
            </View>
          </View>
